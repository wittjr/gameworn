...
Workflow:
  ...
  6. Register the spec at the bottom (``register('NHL', '2024-YY', ...)``)
     and add ``import parsers_2024`` to ``memorabilia/meigray/__init__.py``.
...
# or spelled-out 'Month D, 2024') via apply_promo_color_dates.
...
# ---------------------------------------------------------------------------
def build_schedule_2024(wb, actual, manifest, season, tag_teams):
    rows = list(wb[sheet(actual, "SET DATES - JERSEY SCHEDULES")].iter_rows(values_only=True))
    schedule = toolbox.parse_games_schedule(rows, season, tag_teams)
...
def _apply_color_corrections(entries, schedule):
    ...
    for e in entries:
        ...
        if set_num is not None:
            e.games_worn = toolbox.lookup_games(schedule, e.team, new_color, set_num, set_raw=e.set_number)
...
# ---------------------------------------------------------------------------
def parse_tags_2024(rows, col, header_idx, headers, league, schedule, season, report):
    from memorabilia.models import MeiGrayEntry
    
    entries, total = toolbox.parse_entries_color_set(rows, col, header_idx, league, schedule, report, MeiGrayEntry)
...
# ---------------------------------------------------------------------------
def corrections_2024(entries):
    toolbox.apply_promo_color_dates(entries, extras=_PROMO_EXTRAS)


SPEC_2024 = YearSpec(
    manifest=MANIFEST_2024,
    build_schedule=build_schedule_2024,
    parse_tags=parse_tags_2024,
    corrections=corrections_2024,
)

register("NHL", "2024-25", SPEC_2024)
